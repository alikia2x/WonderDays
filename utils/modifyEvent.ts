type modifyEventResult = {
    code: number;
    details: string;
};

export default function (name: string, data: CountdownEvent): modifyEventResult {
    ...
    localStorage["events"] = JSON.stringify([...parsedEvents.filter((event) => event.name !== name), data]);
